...
(function (factory) {
   ...
   var EMPTY_PACK = { icons: [], lookup: Object.create(null) };
   var INSTANCE_SEQ = 0;
   ...
   /**
    * IconStore owns normalized metadata and URL-level caches.
    * It intentionally has no DOM responsibilities.
    */
   var IconStore = {
      globalIcons: [],
      ...
      normalize: function (source, useCache) {
         ...
         var pack = { icons: icons, lookup: lookup };
   ...
   /**
    * Renderer owns creation/update of the picker UI.
    */
   var Renderer = {
      createPopover: function (options) {
         ...
         popover.removeClass(
            'in inline topLeftCorner topLeft top topRight topRightCorner rightTop right rightBottom bottomRight bottomRightCorner bottom bottomLeft bottomLeftCorner leftBottom left leftTop',
         );
         ...
         return { popover: popover, picker: picker };
      },
   ...
   /**
    * Positioner maps named placements to the lightweight positioning contract.
    */
   var Positioner = {
      classes:
      ...
      apply: function (instance, placement, collision) {
         ...
         var containerOffset = instance.container.offset() || { left: 0 };
   ...
   /**
    * SharedPopover guarantees at most one floating picker UI at a time.
    */
   var SharedPopover = {
      owner: null,
      popover: $(),
      ...
      uiSignature: function (instance) {
         ...
         return [
            ...
            templates.iconpickerItem,
            options.title,
            options.showFooter,
            options.searchInFooter,
            options.inputSearch,
            options.animation,
         ].join('||');
      },
      ...
      bindUiEvents: function () {
         ...
         this.popover
            .off(EVENT_NS)
            .on('click' + EVENT_NS, '.iconpicker-item', function (event) {
   ...
   /**
    * GlobalEventManager installs one window/document listener set for all instances.
    */
   ...
   IconPicker.defaults = {
      ...
      templates: {
         ...
         buttons:
            '<button type="button" class="iconpicker-btn iconpicker-btn-cancel btn btn-default btn-sm">Cancel</button> <button type="button" class="iconpicker-btn iconpicker-btn-accept btn btn-primary btn-sm">Accept</button>',
   ...
   IconPicker.prototype = {
      ...
      _resolveComponent: function () {
         var component = this.isDropdownMenu()
            ? this.container.parent().find(this.options.component).first()
            : this.container.find(this.options.component).first();
         return component.length ? component : false;
      },

      _applyInitialIconPack: function () {
         var source =
            this.options.icons && this.options.icons.length
               ? this.options.icons
               : IconStore.globalIcons.length
                 ? IconStore.globalIcons
                 : (typeof window !== 'undefined' && window.FontAwesomeIconPickerIcons) || [];
      ...
      _loadIcons: function () {
         ...
         this._iconsLoading
            .done(function (pack) {
               ...
               self._trigger('iconpickerIconsLoaded', { iconCount: self._pack.icons.length });
            })
            .fail(function (error) {
               if (self._destroyed) {
                  return;
               ...
               self._iconsLoading = null;
               self._trigger('iconpickerIconsLoadError', { error: error });
            });
      ...
      setValue: function (value) {
         ...
         if (valid === false) {
            this._trigger('iconpickerInvalid', { iconpickerValue: value });
            return false;
         }

         this.iconpickerValue = valid;
         this._trigger('iconpickerSetValue', { iconpickerValue: valid });
         return valid;
      },
      ...
      setSourceValue: function (value) {
         ...
         this._trigger('iconpickerSetSourceValue', { iconpickerValue: valid });
         return valid;
      },
      ...
      show: function () {
         ...
         nextTick(
            function () {
               ...
               self._trigger('iconpickerShown', { iconpickerValue: self.iconpickerValue });
            },
      ...
      hide: function () {
         ...
         nextTick(
            function () {
               ...
               self._trigger('iconpickerHidden', { iconpickerValue: self.iconpickerValue });
            },
            this.options.animation ? 300 : 0,
         );
